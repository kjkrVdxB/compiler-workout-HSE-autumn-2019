...
let rec eval env ((cs, st, ((s, i, o) as c)) as config) prg =
    ...
    | (f :: rest) ->
        ...
        | ENTER ls ->
            eval env (cs, st, (State.push s State.undefined ls, i, o)) rest
        | LEAVE ->
...
let compile dt =
  ...
  | Expr.Call (name, args) -> List.flatten (List.map expr args) @ [CALL (name, List.length args, false)]
  in
  ...
  in
  let rec assign_pt = function
    | Wildcard -> [DROP] (* Drop local pattern *)
    | Ident x  -> [ST x]
    | Sexp (c, pts) -> 
      List.flatten (List.map (fun pt, i -> 
        [DUP; CONST i; CALL (".elem", 2, false)] @ assign_pt pt
      ) (List.combine pts (List.init (List.length pts) (fun x -> x)))) @ [DROP] (* Drop local pattern *)
  in
  let pattern pt faillabel =
    check_pt pt faillabel [] @
    [ENTER (vars pt)] @
    [DUP] @ (* Dupe root for assign_pt *)
    assign_pt pt @
  ...
  and compile' outlabel t = match t with
    ...
    | Stmt.Case (e, pts) ->
      ...
      List.flatten (List.map (fun (pt, b) -> 
        ...
        pattern pt faillabel @ body @ [LEAVE] @ [JMP outlabel] @ [LABEL faillabel; DROP]) pts) (* Drop local pattern from before JMP faillabel *)
    | Stmt.Call (name, args) -> false, List.flatten (List.map expr args) @ [CALL (name, List.length args, true)]
